...
public class AuthService {
                ...
                refreshTokenRepository.findByUserId(user.getId())
                                .ifPresentOrElse(
                                                ...
                                                () -> refreshTokenRepository.save(
                                                                RefreshToken.create(user, refreshToken, expiresAt)));
        ...
        @Transactional
        public AuthResult reissue(ReissueRequest request) {
                Claims claims;
                try {
                ...
                TokenResponse tokenResponse = new TokenResponse(newAccessToken, newRefreshToken, "Bearer",
                                jwtTokenProvider.getAccessTokenExpiration());
                UserResponse userResponse = new UserResponse(user.getId(), user.getEmail(), user.getNickname(),
                                user.getRole(), user.getProfileImageUrl());
                return new AuthResult(tokenResponse, userResponse);
